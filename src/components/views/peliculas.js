...
//Creo un componente clase para manejar la vista de todas las películas
class Peliculas extends React.Component {

    //Defino el construtor
    constructor() {
        super();
        this.state = { _id: '', titulo: '', duracion: '', año: '', descripcion: '', imagen: '', peliculas: [] };
    ...
    addMovie(ev) {
        ...
        if (this.state._id) {
            ...
            fetch(`http://localhost:4200/peliculas/update/${this.state._id}`, {
                method: "POST",
            ...
            fetch("http://localhost:4200/peliculas/addMovie", {
                method: "POST",
                body: JSON.stringify(this.state),
                headers: {
                    'Accept': 'application/json',
    ...
    getMovies() {
        fetch("http://localhost:4200/peliculas")
            .then(res => res.json())
            .then(data => {
    ...
    editMovies(id) {
        fetch(`http://localhost:4200/peliculas/${id}`)
            .then(res => res.json())
            .then(data => {
                this.setState({
                    _id: data._id,
    ...
    render() {
        return (
            <>
                <h1 className="titulo-peliculas">Peliculas</h1>

                {/**Formulario para agregar ls peliculas */}
                            ...
                            <>
                                <div className="tarjetas-container">
                                    <div className="botones">
                                        <button className="botonEditar btn btn-warning" onClick={() => this.editMovies(pelicula._id)}><i className="fas fa-edit"></i></button>
                                        <button className="botonBorrar btn btn-danger" onClick={() => this.deleteMovie(pelicula._id)}><i className="fas fa-times-circle"></i></button>
                                    </div>
                                    <div className="tarjetas">
                                        <Cards key={pelicula._id} pelicula={pelicula} />
                                    </div>
                                </div>
                            </>
                        )
